import { c as create_ssr_component, v as validate_component, d as createEventDispatcher, e as escape, n as null_to_empty, f as add_attribute } from "./index2.js";
...
const Button = create_ssr_component(($$result, $$props, $$bindings, slots) => {
  ...
  $$result.css.add(css);
  return `${isButton ? `<button ${disabled ? "disabled" : ""} class="${escape(null_to_empty(clsx(style)), true) + " s-R7G0-bQFHUk3"}"><span class="${escape(null_to_empty(clsx({ hidden: chooseType == "only-icon" })), true) + " s-R7G0-bQFHUk3"}">${escape(message)}</span>
		${validate_component(IconsRender, "IconsRender").$$render($$result, { icon }, {}, {})}</button>` : `<a${add_attribute("href", href, 0)} class="${escape(null_to_empty(clsx(style)), true) + " s-R7G0-bQFHUk3"}"><span class="${escape(null_to_empty(clsx({ hidden: chooseType == "only-icon" })), true) + " s-R7G0-bQFHUk3"}">${escape(message)}</span>
